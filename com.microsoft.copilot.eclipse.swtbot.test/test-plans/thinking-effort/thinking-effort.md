...
## TC-003: Different effort levels produce noticeably different thinking depth

**Type:** `Happy Path`
**Priority:** `P1`

#### Preconditions
- A reasoning-capable model is selected (e.g. Claude Sonnet).
- The model picker hover card is accessible.

#### Steps
1. Set the effort to **`Low`** via the hover card.
2. Send: _"Is free will compatible with determinism? Think through this carefully."_
3. Observe the collapsed **Thinking** block — note its length / the time it
   takes to stream.
4. Start a new chat session. Set the effort to **`High`**.
5. Send the exact same question.
6. Compare the Thinking block from step 3 with the one from step 5.

#### Expected Result
- The `High` effort Thinking block is substantially longer and more detailed
  than the `Low` effort one, reflecting a deeper reasoning process.
- Both responses ultimately answer the question, but the depth of the thinking
  trace differs visibly.
